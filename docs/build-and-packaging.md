...
On Gentoo, install the equivalent prerequisites with:
...
The deb package uses the upstream dependency baseline. RPM, pacman, and Gentoo templates
...
`make package` detects deb, RPM, pacman, or Gentoo from `/etc/os-release` and available
...
`make gentoo` builds an `app-misc/codex-desktop` Portage binary package using a
temporary local repository and the current GPKG format. The package can be
installed with `./scripts/install-gentoo.sh`; no permanent overlay or binhost
configuration is written. Updater-enabled builds use OpenRC per-user services;
set `PACKAGE_WITH_UPDATER=0` on older OpenRC installations.
