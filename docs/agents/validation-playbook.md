...
```bash
bash -n install.sh scripts/lib/*.sh launcher/start.sh.template scripts/build-gentoo.sh scripts/install-gentoo.sh scripts/user-service.sh
...
Inspect deb, RPM, pacman, Gentoo GPKG, AppImage, and Nix outputs on both architectures. Check
...
On Gentoo, index the generated GPKG through Portage and exercise the dry-run
installer (`CODEX_GENTOO_INSTALL_DRY_RUN=1 ./scripts/install-gentoo.sh ...`).
For OpenRC, verify `rc-service --user codex-update-manager status` and the
user `default` runlevel enablement.
