...
On a supported Debian/Ubuntu, Fedora, openSUSE, Arch-derived, Gentoo/OpenRC, or
...
On Gentoo, auto-detection builds a Portage GPKG. Updater-enabled packages
require OpenRC 0.62+ for per-user services; use `PACKAGE_WITH_UPDATER=0` for
older OpenRC installations.
...
On Gentoo/OpenRC, use `rc-service --user codex-update-manager status` and
`rc-update --user show default` instead.
