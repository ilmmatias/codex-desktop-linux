...
.PHONY: help check test ci-pr ci-all build-updater maybe-build-updater build-native-feature-helpers update rebuild rebuild-install inspect-upstream build-app build-app-fresh setup-native bootstrap-native install-native update-native rebuild-next run-app deb rpm pacman gentoo appimage package install service-enable service-status clean-dist clean-state
...
help:
	...
	@printf '  %-20s %s\n' 'make gentoo' 'Build a Portage GPKG in dist/'
...
package:
	...
	case "$$format" in \
	  ...
	  gentoo) $(MAKE) gentoo PACKAGE_WITH_UPDATER="$(PACKAGE_WITH_UPDATER)" ;; \
...
install:
	...
	case "$$format" in \
	  ...
	  gentoo) artifact="$${GENTOO:-$$(latest '$(GENTOO_GLOB)')}"; [ -n "$$artifact" ]; "$(CURDIR)/scripts/install-gentoo.sh" "$$artifact" ;; \
