...
| Gentoo (OpenRC) | `make bootstrap-native` | Builds and installs a Portage `.gpkg.tar` |
...
| Native deb, RPM, pacman, and Gentoo packages | Manual build | `make deb`, `make rpm`, `make pacman`, or `make gentoo` |
...
Gentoo uses OpenRC's per-user service mode instead:
