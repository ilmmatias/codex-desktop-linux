...
fn run_privileged_command(command: &Commands) -> Option<Result<()>> {
    match command {
        ...
        Commands::InstallGentoo { path } => Some(install::install_gentoo(path)),
        ...
        Commands::InstallRollbackGentoo { path } => Some(install_rollback::install_gentoo(path)),
        _ => None,
    }
}
