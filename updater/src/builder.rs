...
pub async fn build_update(
    ...
    state.save_updater(&paths.state_file)?;
    let (script, service_manager, service_source, helper_source, runtime_source, cleanup_source) = match PackageKind::detect() {
        PackageKind::Deb => (
            "scripts/build-deb.sh",
            "systemd",
        ...
        PackageKind::Gentoo => (
            "scripts/build-gentoo.sh",
            "openrc-user",
            "packaging/linux/codex-update-manager.openrc",
            "packaging/linux/codex-update-manager-openrc-user-service.sh",
            "packaging/linux/codex-packaged-runtime-openrc.sh",
    ...
    package
        ...
        .env("PACKAGED_RUNTIME_SOURCE", bundle.join(runtime_source))
        .current_dir(&bundle);
    if !cleanup_source.is_empty() {
        package.env("NO_UPDATER_CLEANUP_SOURCE", bundle.join(cleanup_source));
    }
    if let Some(config_path) = effective_feature_config_path(config) {
...
fn find_package(dist: &Path) -> Result<PathBuf> {
    ...
    for entry in fs::read_dir(dist).with_context(|| format!("Failed to read {}", dist.display()))? {
        ...
        if name.ends_with(".deb") || name.ends_with(".rpm") || name.contains(".pkg.tar.") || name.ends_with(".gpkg.tar") {
            matches.push(path);
        }
    }
...
mod tests {
    ...
    #[test]
    fn finds_gentoo_gpkg_in_dist() -> Result<()> {
        let root = std::env::temp_dir().join(format!("codex-builder-gpkg-{}", std::process::id()));
        fs::create_dir_all(&root)?;
        let package = root.join("codex-desktop-2026.08.31.gpkg.tar");
        fs::write(&package, b"gpkg")?;
        assert_eq!(find_package(&root)?, package);
