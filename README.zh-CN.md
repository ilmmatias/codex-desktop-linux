...
| Gentoo（OpenRC） | `make bootstrap-native` | 构建并安装 Portage `.gpkg.tar` |
...
| deb、RPM、pacman、Gentoo 原生包 | 手动构建 | `make deb`、`make rpm`、`make pacman`、`make gentoo` |
...
Gentoo 使用 OpenRC 的 per-user service：
