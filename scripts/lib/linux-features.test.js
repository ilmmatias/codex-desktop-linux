...
test("Gentoo package format accepts feature resources and dependencies", (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "codex-feature-package-gentoo-"));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const { featureDir, featuresRoot } = makePackageFeatureRoot(root, {
    id: "package-framework-fixture",
    title: "Package Framework Fixture",
    packageResources: [{ source: "payload.txt", target: "usr/share/codex/payload.txt", formats: ["gentoo"] }],
    packageDependencies: { gentoo: ["app-misc/fixture-runtime"] },
  });
  fs.writeFileSync(path.join(featureDir, "payload.txt"), "payload\n");
  fs.writeFileSync(path.join(featuresRoot, "features.json"), '{"enabled":["package-framework-fixture"]}\n');
  const plan = enabledLinuxFeaturePackagePlan({ featuresRoot, packageFormat: "gentoo" });
  assert.deepEqual(plan.dependencies, ["app-misc/fixture-runtime"]);
  assert.equal(plan.resources[0].target, "usr/share/codex/payload.txt");
});
