...
const SUPPORTED_PACKAGE_FORMATS = new Set(["deb", "rpm", "pacman", "gentoo"]);
...
function main() {
  ...
  if (command === "--enabled-feature-dirs0") {
    for (const feature of loadEnabledLinuxFeatures()) {
      process.stdout.write(`${feature.id}\0${feature.relativeDir}\0`);
    }
    return;
  }
